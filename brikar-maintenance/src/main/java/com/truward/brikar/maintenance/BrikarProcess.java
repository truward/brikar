...
public final class BrikarProcess implements AutoCloseable {
  ...
  public static final class Builder {
    ...
    public Builder addTempConfiguration(Map<String, String> properties) {
      final Map<String, String> config = new HashMap<>(properties);
      if (!config.containsKey(StandardLauncher.CONFIG_KEY_PORT)) {
        // no port - use random one
        final int port = LaunchUtil.getAvailablePort();
        config.put(StandardLauncher.CONFIG_KEY_PORT, Integer.toString(port));
      ...
      try {
        ...
        try (final FileOutputStream fileOutputStream = new FileOutputStream(tempConfigFile)) {
          props.store(fileOutputStream, "Autogenerated properties by BrikarProcess");
        }

        configFilePath = tempConfigFile.toURI().toURL().toExternalForm();
      } catch (IOException e) {
        throw new IllegalStateException(e);
      }

      return addDefine(StandardLauncher.SYS_PROP_SETTINGS_OVERRIDE, configFilePath);
    }
